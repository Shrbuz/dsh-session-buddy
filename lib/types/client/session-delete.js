...
export function readSessionIdFromRow(row) {
    let fiber = null;
        ...
        if (entry !== undefined && entry.corrupt) {
            ...
            if (marker === null) {
                ...
                if (slot !== null) {
                    ...
                    if (title !== null)
                        title.parentElement?.insertBefore(marker, title.nextSibling);
                    else
...
function ensureDeleteMenuItem(container, onDelete) {
    if (injectedDeleteItem !== null && injectedDeleteItem.parentElement === container)
        return injectedDeleteItem;
    ...
    const template = Array.from(container.children).find((child) => child instanceof HTMLElement
...
export function startSessionDeleteManager(options = {}) {
    ...
    const refreshHealth = async () => {
        ...
        for (const [id, entry] of fresh)
            corrupt.set(id, entry);
        applyMarkers();
    };
    const applyMarkers = () => {
        const rows = document.querySelectorAll('div[role="treeitem"][class$="_sessionRow"]');
    ...
    return () => {
        ...
        removeDeleteMenuItem();
        for (const el of Array.from(document.querySelectorAll(`[${CORRUPT_ATTR}]`)))
            el.remove();
        for (const el of Array.from(document.querySelectorAll('[data-dsb-modal]')))
            el.remove();
        style.remove();
    };
}
