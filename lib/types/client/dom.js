 ...
 * @param max - max chars.
 */
export function firstLineSummary(el, max = 60) {
...
export function collectRungs(root = document) {
    const rungs = [];
...
export function isStreaming(root = document) {
    if (root.querySelector('[data-composer-seat] button[class*="primary"] svg rect') !== null)
        return true;
    return root.querySelector('[data-streaming]') !== null;
}
/** Whether the composer seat is present (i.e. a session is open and editable). */
export function hasComposerSeat(root = document) {
...
export function anchorRowByKey(root, key) {
 ...
 */
export function readSessionTitle(root = document) {
