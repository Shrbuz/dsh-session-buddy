...
import { SessionBuddySettingsCard } from "./SessionBuddySettingsCard.js";
import { sessionBuddySettingsSpec } from "./settings.js";
import { t } from "./locales.js";
import { BUDDY_CSS } from "./styles.js";
import { startSessionListener } from "./listener.js";
import { notify } from "./notifier.js";
import { anchorRowByKey } from "./dom.js";
import { createSessionSource, alignRungKeys } from "./session-source.js";
import { startBuddyEventStream, wasHiddenSince } from "./sse.js";
import { startSessionDeleteManager } from "./session-delete.js";
import { startCollapseTools } from "./collapse-tools.js";
...
function currentWorkspaceName(sessions) {
    try {
        ...
        if (cwd === undefined || cwd === '')
            return undefined;
        if (typeof workspaceTitleOf !== 'function')
            return undefined;
...
export function apply(ctx) {
    ...
    const renderOutline = () => {
        if (!outlineMounted || root === undefined)
            return;
    ...
    const dispatch = (event, claimKey) => {
        ...
        if (current === undefined || !current.enabled)
            return;
        if (!current[switchOf(event.kind)])
            return;
        ...
        if (sseDispose === undefined) {
            sseDispose = startBuddyEventStream({
                ...
                onTrigger: (trigger) => {
                    ...
                    if (trigger.sessionId !== currentSessionId())
                        return;
                    dispatch({
                        kind: trigger.kind,
