...
export function OutlinePanel(props) {
    ...
    // poll until it appears. Repositioning is EVENT-DRIVEN (ResizeObserver on the
    // scrollport AND its ancestors + window resize + a DOM-level MutationObserver
    // backstop), coalesced to at most one re-position per frame — so the rail
    // follows the conversation width deterministically with zero idle cost.
    useEffect(() => {
        const GAP = 12;
        const total = 8 + 16 + 2;
        let ro;
        ...
        // Coalesce bursts of events (resize, DOM churn) into one re-position.
        const schedule = () => {
            if (pending)
                return;
            pending = true;
        ...
        const attach = () => {
            ...
            if (ro === undefined) {
                // Observe the scrollport AND every ancestor up to (but not including)
                // body: whichever container the sidebar squeeze actually resizes will
                // fire the observer, so the rail repositions without missing a beat.
                ...
                // DOM-level backstop: a sidebar toggle that swaps content (rather than
                // resizing a box we observe) still triggers a re-position.
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("aside", { className: "dsb-outline", "data-dsh-part": "outline", ref: railRef, style: { left }, "aria-label": t('buddy.outline.title'), children: [_jsx("div", { className: [
                            'dsb-outline-list',
                            canScrollTop ? 'dsb-outline-list-can-top' : '',
                            canScrollBottom ? 'dsb-outline-list-can-bottom' : '',
                        ].join(' '), "data-dsh-part": "outline-list", ref: scrollRef, role: "list", "aria-label": t('buddy.outline.title'), style: { '--dsb-rung-h': `${railWidth}px` }, children: rungs.map((rung, index) => (_jsx("button", { type: "button", role: "listitem", className: [
                                'dsb-outline-rung',
