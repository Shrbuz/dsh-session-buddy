...
const visibilityTransitions = [{ at: Date.now(), hidden: document.hidden }];
function recordVisibility() {
    ...
    if (visibilityTransitions.length > 512)
        visibilityTransitions.splice(0, visibilityTransitions.length - 512);
}
 ...
 * for unit tests.
 */
export function wasHiddenSince(start, now = Date.now()) {
    if (start > now)
        return false;
 ...
 */
export function startBuddyEventStream(options) {
