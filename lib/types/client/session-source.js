...
export function collectRungsFromSnapshot(snapshot) {
...
export function alignRungKeys(rungs) {
    ...
    for (const row of domRows) {
        ...
        if (text !== '')
            byText.set(text.slice(0, 40), key);
    }
...
export function currentSessionFace(sessions) {
 ...
 */
export function createSessionSource(sessions, handlers) {
    let currentFace;
    ...
    const refreshCurrent = () => {
        ...
        if (currentFace !== undefined && currentFace.sessionId === id)
            return;
    ...
    const loadOlderOnce = async () => {
        if (currentFace === undefined)
            return;
        const snapshot = currentFace.getSnapshot();
        if (!(snapshot.hasMore ?? false) || snapshot.loadingOlder === true)
            return;
        await currentFace.loadOlder();
        await new Promise((resolve) => setTimeout(resolve, 200));
    };
