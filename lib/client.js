		...
		function OutlinePanel(props) {
			...
			(0, react.useEffect)(() => {
				...
				const update = () => {
					...
					const left = Math.max(rect.left + 4, rect.right - total - GAP);
					if (left !== lastLeft) {
						lastLeft = left;
						setLeft(left);
					}
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsb-settings-group-label",
							children: buddyT("buddy.settings.view")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Toggle, {
							...
							onChange: (next) => {
								set("collapseTools", next);
							}
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsb-settings-field",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsb-settings-label dsb-settings-label-desc",
								children: buddyT("buddy.settings.collapseToolsDesc")
							})
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Toggle, {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsb-settings-label dsb-settings-label-desc",
								children: buddyT("buddy.settings.foldThinkDesc")
							})
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Toggle, {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsb-settings-label dsb-settings-label-desc",
								children: buddyT("buddy.settings.foldLongUserDesc")
							})
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsb-settings-group-label",
...
[data-dsb-delete-item] svg { color: inherit; }`;
		...
		*/
		...
		//#endregion
		//#region src/client/collapse-tools.ts
		/**
		...
		*/
		function parseTurnFromKey(key) {
			if (typeof key !== "string" || !key.startsWith("9:turn-tail")) return null;
			const turn = Number(key.slice(11));
		...
		function groupToolRuns(rows) {
			...
			rows.forEach((row, index) => {
				if (row.kind === "turn-tail") {
					...
					if (turn !== null && pendingTools.length > 0) groups.push({
						turn,
						steps: pendingTools.length,
						toolRowIndexes: pendingTools.slice()
					});
					pendingTools.length = 0;
				} else if (row.kind === "tool-call") pendingTools.push(index);
			});
			return groups;
		}
		/**
		...
		*/
		function groupThinkRuns(rows) {
			...
			rows.forEach((row, index) => {
				if (row.kind === "turn-tail") {
					const turn = parseTurnFromKey(row.key);
					if (turn !== null && pendingThinks >= 2) groups.push({
						turn,
					...
					pendingContexts = [];
					pendingThinks = 0;
				} else if (row.isContext) pendingContexts.push(index);
		...
		function createBar(t, turn, count, expanded, variant) {
			...
			if (isThink) bar.dataset.dsbThinkBar = "1";
			else bar.dataset.dsbCollapseBar = "1";
		...
		*/
		function startCollapseTools(options) {
			...
			const restoreAll = () => {
				for (const bar of [...document.querySelectorAll("[data-dsb-collapse-bar], [data-dsb-think-bar], [data-dsb-user-bar]")]) bar.remove();
				for (const el of document.querySelectorAll("[data-chat-anchor-key], [data-variant=\"think\"]")) if (el.style.display === "none") el.style.display = "";
			...
			const clearVariant = (selector, owns) => {
				for (const bar of [...document.querySelectorAll(selector)]) bar.remove();
				for (const el of document.querySelectorAll("[data-chat-anchor-key], [data-variant=\"think\"]")) if (owns(el) && el.style.display === "none") el.style.display = "";
			};
			/** Remove long-question bars and restore every clamped text container. */
			const clearLongUser = () => {
				for (const bar of [...document.querySelectorAll("[data-dsb-user-bar]")]) bar.remove();
				for (const el of document.querySelectorAll("[class*=\"_text_\"]")) {
					el.style.maxHeight = "";
					el.style.overflow = "";
					el.classList.remove("dsb-user-clamped");
			...
			const foldTools = (sessionId, specs, elements) => {
				...
				for (const [turn, bar] of existing) if (!keptTurns.has(turn)) bar.remove();
				for (const group of groups) {
					const toolEls = group.toolRowIndexes.map((index) => elements[index]).filter((el) => el !== void 0);
					if (toolEls.length === 0) continue;
					const isExpanded = expanded.has(`${sessionId ?? ""}#${group.turn}`);
					let bar = existing.get(group.turn);
					if (bar === void 0 || bar.isConnected === false) {
					...
					for (const el of toolEls) el.style.display = isExpanded ? "" : "none";
				}
			};
			...
			*  summary and stays visible — only its think block folds. */
			const foldThinks = (sessionId, thinkSpecs, elements) => {
				...
				for (const group of groups) {
					...
					if (group.finalStepIndex !== null) {
						...
						if (finalRow !== void 0) finalRow.querySelectorAll("[data-variant=\"think\"]").forEach((thinkEl, ti) => {
							items.push({
								el: thinkEl,
								order: group.finalStepIndex * 1e3 + ti
							});
						});
					}
					...
					if (bar === void 0 || bar.isConnected === false) {
						...
						(foldedEls[0].closest("[data-chat-anchor-key]") ?? foldedEls[0]).insertAdjacentElement("beforebegin", bar);
						existing.set(group.turn, bar);
					}
					bar.setAttribute("aria-expanded", String(isExpanded));
					...
					bar.classList.toggle("dsb-collapse-open", isExpanded);
					for (const el of foldedEls) el.style.display = isExpanded ? "" : "none";
				}
			};
			...
			const foldLongUser = (sessionId) => {
				...
				for (const bar of document.querySelectorAll("[data-dsb-user-bar]")) {
					...
					if (key !== void 0) existing.set(key, bar);
				}
				const seen = /* @__PURE__ */ new Set();
				for (const row of document.querySelectorAll("[data-chat-anchor-key]")) {
					if (row.getAttribute("data-chat-flow-kind") !== "user") continue;
					const key = row.getAttribute("data-chat-anchor-key");
					if (key === null) continue;
					seen.add(key);
					const textEl = [...row.querySelectorAll("[class*=\"_text_\"]")].filter((el) => el.offsetHeight > 0).sort((a, b) => b.offsetHeight - a.offsetHeight)[0];
					if (textEl === void 0) continue;
					const lineHeight = Number.parseFloat(getComputedStyle(textEl).lineHeight);
					const maxHeight = (Number.isFinite(lineHeight) && lineHeight > 0 ? lineHeight : 24) * 6;
				...
				for (const [key, bar] of existing) if (!seen.has(key)) bar.remove();
			};
			...
			const apply = () => {
				...
				if (enabled) foldTools(sessionId, specs, elements);
				else clearVariant("[data-dsb-collapse-bar]", (el) => el.getAttribute("data-chat-flow-kind") === TOOL_CALL_KIND);
				if (thinkEnabled) foldThinks(sessionId, thinkSpecs, elements);
				else clearVariant("[data-dsb-think-bar]", (el) => {
					const kind = el.getAttribute("data-chat-flow-kind");
					return kind === "context" || kind === "assistant-step" || el.matches("[data-variant=\"think\"]");
				});
				if (longUserEnabled) foldLongUser(sessionId);
				else clearLongUser();
			};
			let scheduled = false;
			const observer = new MutationObserver(() => {
		...
		function apply(ctx) {
			...
			*  off to notifier with an optional cross-tab claim key. */
			...
			const syncEnabled = () => {
				...
				if (collapseDispose === void 0) collapseDispose = startCollapseTools({
					isEnabled: () => {
						const current = settingsScope.getSnapshot().value;
