	...
	const versionRoute = {
		...
		handler: async (request, response) => {
			...
			writeJson(response, 200, {
				...
				current: LIB_VERSION,
				latest,
				updateAvailable: latest !== void 0 ? (compareVersions(latest, "0.1.2") ?? 0) > 0 : false
			});
		}
	};
	const updateRoute = {
...
//#endregion
export { LIB_VERSION, PACKAGE_NAME, SESSION_BUDDY_NAMESPACE, TOAST_ROUTE, apply, compareVersions, findDshBinary, inject, makeSessionBuddySettingsSchema, name, parseVersion, resolveLaunch, runDshCli, unsafeSpecReason };
