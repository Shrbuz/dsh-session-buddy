...
function readClaims(file: string): Map<string, number> {
  try {
 ...
 */
export function tryClaimNotification(claimKey: string, file = LEDGER_FILE): boolean {
  if (claimKey === '') return true // no dedup requested → always allow
  ...
  let pruned: Array<[string, number]> = []
  for (const [key, ts] of claims) {
    if (now - ts <= TTL_MS) pruned.push([key, ts])
  }
