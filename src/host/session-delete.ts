...
/** A minimal view of the persistence service (avoids pulling its types). */
...
export function decodeSessionRows(buf: Buffer): { rows: unknown[]; decodeError: string | null } {
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    for (const line of dec.split('\n')) {
      ...
      try { rows.push(JSON.parse(line)) } catch { return { rows, decodeError: 'json-line' } }
    }
  }
...
export function detectCorruption(rows: readonly unknown[]): { corrupt: boolean; reason?: string } {
  const messageTypes = new Set(['user/message', 'assistant/message', 'tool/result'])
  for (const raw of rows) {
    const event = raw as { type?: string; seq?: number; time?: number; data?: unknown }
    ...
    if (
      typeof event.type !== 'string' || typeof event.seq !== 'number' || !Number.isSafeInteger(event.seq)
      || event.seq < 0 || typeof event.time !== 'number' || event.data === undefined
    ) return { corrupt: true, reason: `envelope at seq ${event.seq}` }
    const record = event.data as { message?: unknown; role?: unknown }
    const message = (event.type === 'user/message' ? record : record?.message) as
    ...
    if (message.role !== expectedRole) return { corrupt: true, reason: `role at seq ${event.seq}` }
    const source = message.source as { kind?: unknown; callId?: unknown; provider?: unknown; model?: unknown } | null
    ...
    if (!Array.isArray(message.content)) return { corrupt: true, reason: `content at seq ${event.seq}` }
    ...
    const content = message.content as Array<{ type?: unknown; toolCallId?: unknown; content?: unknown }>
    ...
    if (block.toolCallId !== source.callId) return { corrupt: true, reason: `mismatched tool call ids at seq ${event.seq}` }
  }
  return { corrupt: false }
}

/** Detect corruption from the raw artifact bytes (decode + validate). */
export function detectCorruptionInLog(buf: Buffer): { corrupt: boolean; reason?: string } {
  const { rows, decodeError } = decodeSessionRows(buf)
  if (decodeError !== null) return { corrupt: true, reason: `decode:${decodeError}` }
...
function sessionDirInfo(persistence: PersistenceLike, header: { id: string; cwd?: string }): { dir: string; log: string; size: number } | undefined {
  const located = persistence.locate(header)
  if (located === undefined || located.kind !== 'jsonl') return undefined
  const log = located.path
  const dir = dirname(log)
  if (log === dir) return undefined // malformed locate
  const base = log.endsWith(LOG_SUFFIX_ZSTD) ? LOG_SUFFIX_ZSTD : LOG_SUFFIX_PLAIN
  if (!log.endsWith(base)) return undefined
 ...
 */
export async function listSessions(ctx: Context): Promise<SessionHealthEntry[]> {
...
function liveSessionIds(ctx: Context): Set<string> {
  const store = (ctx as unknown as { get?: (name: string) => unknown }).get?.('sessions') as
    | { list?: () => unknown[] }
    | undefined
  const out = new Set<string>()
  // Call as a method (not detached) so `this` stays bound to the store.
  if (store === undefined || typeof store.list !== 'function') return out
  for (const s of store.list()) {
    const header = (s as { header?: { id?: unknown } })?.header
 ...
 */
export async function deleteSession(ctx: Context, sessionId: string): Promise<SessionDeleteResult> {
  const persistence = (ctx as unknown as { get?: (name: string) => unknown }).get?.('sessionPersistence') as PersistenceLike | undefined
  ...
  if (header === undefined) return { ok: false, id: sessionId, error: 'session-not-found' }
  if (liveSessionIds(ctx).has(sessionId)) return { ok: false, id: sessionId, error: 'session-live' }
  const info = sessionDirInfo(persistence, header)
  if (info === undefined) return { ok: false, id: sessionId, error: 'no-artifact' }
