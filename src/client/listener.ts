 ...
 */
export function startSessionListener(options: SessionListenerOptions): () => void {
  const state: ClassifierState = createClassifierState()
  ...
  function rebuildAndClassify(): void {
    ...
    let latestReplySummary: string | undefined
    let latestSettledKey: string | undefined
  ...
  function armSettleCheck(): void {
    if (settleTimer !== undefined || disposed) return
  ...
  const onMutation = (): void => { rebuildAndClassify() }
  ...
  let scrollportObserver: MutationObserver | undefined
  function ensureScrollport(): void {
  ...
  const onVisibilityChange = (): void => { rebuildAndClassify() }
