...
/** Persisted marker state: remembers the original title & favicon so the
 * marker can be cleanly removed, and whether we created the icon link. */
interface MarkerState {
  originalTitle: string
  /** The icon <link> we modify (existing or one we created). */
  icon: HTMLLinkElement
  /** Original icon href, if the link pre-existed. */
  originalHref: string | undefined
  /** Whether we created the icon link (to remove it on clear). */
  createdIcon: boolean
}

/** Current marker state, or undefined when no marker is active. */
let marker: MarkerState | undefined
